...
function zcf_callcontactform7mapping($formtype) {
    ...
    global $formattr;
    global $attrname;
    global $module_options, $module, $isWidget, $assignedto, $check_duplicate, $update_record,$assignedto;
    $plugin_dir = ZCF_BASE_DIR_URI;
    ...
    if (isset($data) && $data) {
        ...
        if (sanitize_text_field(isset($_REQUEST['submitcontactformwidget']))) {
            $formnumber = sanitize_text_field($_REQUEST['formnumber']);
            $submitcontactform = "widgetcrmformsLogMsg{$formnumber}";
        }
        $successfulAttemptsOption['total'] = isset($config_fields['submit_count'])?$config_fields['submit_count']:'';
        $successfulAttemptsOption['success'] = isset($config_fields['success_count'])?$config_fields['success_count']:'';
        $total = 0;
        $success = 0;
...
function zcf_ContactFormfieldlistsMapping($module, $config_fields, $module_options, $formtype, $thirdparty) {
    ...
    if (sanitize_text_field(isset($_POST['submitcontactform'])) && (sanitize_text_field($_POST['formnumber']) == sanitize_text_field($_SESSION['generated_forms']))) {
        ...
        for ($i = 0; $i < count($config_fields); $i++) {
            if (array_key_exists($config_fields[$i]['name'], __($_POST))) {
                ...
                if ($config_fields[$i]['zcf_mandatory'] == 1 && sanitize_text_field($_POST[$config_fields[$i]['name']]) == "") {
                    ...
                    $count_error++;
                } elseif ($config_fields[$i]['type']['name'] == 'double' && !preg_match('/^([\d]{1,8}.?[\d]{1,2})?$/', sanitize_text_field($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST[$config_fields[$i]['name']]) != "")) {
                    $count_error++;
                    ...
                    $concat = "";
                    $configFieldcount  = sanitize_text_field($_POST[$config_fields[$i]['name']]);
                    for ($index = 0; $index < count($configFieldcount); $index++) {
        ...
        if (($captcha_config['crmforms_recaptcha'] == 'yes') && (isset($save_field_config['google_captcha']) && (($save_field_config['google_captcha']) == 1 ))) {
            $privatekey = $captcha_config['recaptcha_private_key'];
            if (!(isset($_REQUEST['g-recaptcha-response'])) || ( ($_REQUEST['g-recaptcha-response']) == NULL ) || ( ($_REQUEST['g-recaptcha-response']) == "" )) {
                $captcha_error = true;
            }
        }
    }
    ...
    for ($i = 0; $i < count($config_fields); $i++) {
        ...
        if ($config_fields[$i]['publish'] == 1 && $hiddenFieldChk != 1) {
            ...
            if ($fieldtype == "string" || $fieldtype == "text" || $fieldtype == "formula" || $fieldtype == "bigint") {
                ...
                if (sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST['formnumber']) == $_SESSION['generated_forms']) && $count_error != 0)
                ...
                $configfields = sanitize_text_field($_POST[$config_fields[$i]['name']]);
                $htmlcontent1 .= "<script>document.getElementById('{$config_fields[$i]['name']}').value='{$configfields}'</script>";
                ...
                if ($config_fields[$i]['zcf_mandatory'] == 1 && sanitize_text_field($_POST[$config_fields[$i]['name']]) == "") {
                    $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                } elseif (sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && sanitize_text_field($config_fields[$i]['type']['name']) == 'integer' && !preg_match('/^[\d]*$/', sanitize_text_field($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST[$config_fields[$i]['name']]) != "")) {
                    $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                }
                $htmlcontent1 .= "</span></td></tr>";
                $count_selected++;
                ...
                if ($config_fields[$i]['zcf_mandatory'] == 1 && sanitize_text_field($_POST[$config_fields[$i]['name']]) == "") {
                    $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                } elseif (sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && $config_fields[$i]['type']['name'] == 'currency' && !preg_match('/^([\d]{1,8}.?[\d]{1,2})?$/', sanitize_text_field($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST[$config_fields[$i]['name']]) != "")) {
                    $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                }
                $htmlcontent1 .= "</span></td></tr>";
                $count_selected++;
                ...
                if ($config_fields[$i]['zcf_mandatory'] == 1 && sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && sanitize_text_field($_POST[$config_fields[$i]['name']]) == "") {
                    $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                } elseif (sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && $config_fields[$i]['type']['name'] == 'email' && (!preg_match('/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,3})$/', sanitize_text_field($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST[$config_fields[$i]['name']]) != ""))) {
...
if(sanitize_text_field(isset($_POST['submitcontactform'])) && (sanitize_text_field(intval($_POST['formnumber']))) == sanitize_text_field($_SESSION['generated_forms'])){
                ...
                if (sanitize_text_field(isset($_POST['submitcontactform'])) && (sanitize_text_field(intval($_POST['formnumber'])) == $_SESSION['generated_forms'])) {
                    if ($config_fields[$i]['zcf_mandatory'] == 1 && sanitize_text_field($_POST[$config_fields[$i]['name']]) == "") {
                        $htmlcontent1 .= $config_fields[$i]['display_label'] . " cannot be empty";
                    } elseif (sanitize_text_field(isset($_POST[$config_fields[$i]['name']])) && $config_fields[$i]['type']['name'] == 'url' && (!preg_match('/((http|https)\:\/\/)?[a-zA-Z0-9\.\/\?\:@\-=#]+\.([a-zA-Z0-9\.\/\?\:@\-=#])*/', sanitize_text_field($_POST[$config_fields[$i]['name']])) && (sanitize_text_field($_POST[$config_fields[$i]['name']]) != ""))) {
